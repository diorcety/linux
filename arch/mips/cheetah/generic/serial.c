...
DECLARE_TASKLET(cta_uart_tasklet, cta_uart_deliver, (unsigned long)&rs_table[0]); /* tasklet for dsr */
...
static void rs_disable_tx_interrupts (struct serial_state *info)
{
	unsigned long flags;
...
static void rs_enable_tx_interrupts (struct serial_state *info)
{
	unsigned long flags;
...
static void rs_disable_rx_interrupts (struct serial_state *info)
{
	unsigned long flags;
...
static void rs_enable_rx_interrupts (struct serial_state *info)
{
	unsigned long flags;
...
static void transmit_chars(struct serial_state *info)
{
	...
	{
#ifdef CONFIG_CHEETAH_UART_RTSCTS
		if (ctsrts_en && info->tport.tty->hw_stopped)
			break;
#endif
		/* XXX: check fifo full */
		if ((p[URCS>>2]&URCS_TF))
			...
			if (info->x_char)
			{
				...
				|| info->tport.tty->stopped || info->tport.tty->hw_stopped)
				{
					empty=1;
					break;
				}
...
static irqreturn_t cta_cts(int irq, void *data)
{
	...
	if (info) {
		...
		irq_set_irq_type(IRQ_GPIOX, (info->tport.tty->hw_stopped) ? IRQ_TYPE_LEVEL_HIGH : IRQ_TYPE_LEVEL_LOW);
...
static void cta_uart_deliver(unsigned long x)
{
	...
	volatile int *p = (int *) UR_BASE;
	struct serial_state * info = (struct serial_state *)x;
...
static int startup(struct tty_struct *tty, struct serial_state *info)
{
	...
	if (ctsrts_en) {
		...
		irq_set_irq_type(IRQ_GPIOX, (info->tport.tty->hw_stopped) ? IRQ_TYPE_LEVEL_LOW : IRQ_TYPE_LEVEL_HIGH);
	...
	if (info->tport.tty)
		clear_bit(TTY_IO_ERROR, &info->tport.tty->flags);
...
static void rs_set_termios(struct tty_struct *tty, struct ktermios *old_termios)
{
		...
		(tty->termios.c_cflag & CRTSCTS))
	{
		...
		if (0 > request_irq(IRQ_GPIOX, cta_cts, irqflags, "UART CTS", info))
...
#error "unsupported baudrate"
	...
	state->tport.close_delay = 5*HZ/10;
	state->tport.closing_wait = 30*HZ;
	...
	if (0 > request_irq(IRQ_GPIOX, cta_cts, 0, "UART CTS", state))
		panic("Couldn't request IRQ for UART CTS\n");
	disable_irq(IRQ_GPIOX);
#endif
	local_irq_restore(flags);
