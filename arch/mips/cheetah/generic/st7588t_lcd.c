...
#define ST7588T_PAGE_SIZE 8
#define ST7588T_COL_SIZE 132
#define ST7588T_PAGE_FONT_SIZE 22

#define PIN_SDA_GPIO CONFIG_CHEETAH_ST7588T_LCD_GPIO_SDA
#define PIN_SCL_GPIO CONFIG_CHEETAH_ST7588T_LCD_GPIO_SCL

//========================================================================  
// st7588t lcd sw struct 
//========================================================================
...
struct st7588t_lcd_dev
{
	struct miscdevice misc_dev;
	struct delayed_work work;
	struct semaphore sem;
	unsigned char data[ST7588T_PAGE_SIZE][ST7588T_PAGE_FONT_SIZE];
	unsigned char col_data[ST7588T_PAGE_SIZE][ST7588T_COL_SIZE];
	unsigned int cur_page;
	unsigned int cur_word;
	unsigned int cur_col;
};

struct st7588t_lcd_dev *lcd_dev;

void st7588t_lcd_sw_reset(struct st7588t_lcd_dev *dev)
{
	int i, j;
	dev->cur_page = 0;
	dev->cur_word = 0;
	...
	for (i=0; i<ST7588T_PAGE_SIZE; i++)
	{
		for (j=0; j<ST7588T_PAGE_FONT_SIZE; j++)
			dev->data[i][j] = 0x00;
		for (j=0; j<ST7588T_COL_SIZE; j++)
			dev->col_data[i][j] = 0x00;
	}
}

//========================================================================  
// st7588t lcd hw control flows
//========================================================================

#define CMD_SLAVE_ADDRESS 0x7e
#define CMD_COMMAND_START 0x00
#define CMD_DATA_START 0x40
#define CMD_FUNCTION_SET 0x20

#define CMD_SET_PAGE 0x40
#define CMD_SET_COLUMN_UPPER 0xF0
#define CMD_SET_COLUMN_LOWER 0xE0
#define CMD_SET_START_LINE 0x80

#define CMD_COMMAND_RESET 0x03
#define CMD_VLCD_RANGE 0x04
#define CMD_DISPLAY_CONFIG 0x08
#define CMD_BIAS_SYSTEM 0x10
#define CMD_SET_VOP 0x80


static void
st7588t_lcd_i2c_start(void)
{
	gpio_set_value(PIN_SCL_GPIO, 1);
	gpio_set_value(PIN_SDA_GPIO, 1);
	gpio_set_value(PIN_SDA_GPIO, 0);
}

static void
st7588t_lcd_i2c_stop(void)
{
	gpio_set_value(PIN_SCL_GPIO, 1);
	gpio_set_value(PIN_SDA_GPIO, 0);
	gpio_set_value(PIN_SDA_GPIO, 1);
...
static void
st7588t_lcd_i2c_transfer(unsigned char c)
{
	int i = 0;
	for (i=0; i<8; i++)
	{
...
static void
st7588t_lcd_ctrl_start(void)
{
	st7588t_lcd_i2c_start();
	st7588t_lcd_i2c_transfer(CMD_SLAVE_ADDRESS);
}

static void
st7588t_lcd_ctrl_stop(void)
{
	st7588t_lcd_i2c_stop();
}

static void
st7588t_lcd_ctrl_command(unsigned char c)
{
	st7588t_lcd_i2c_transfer(c);
}

static void
st7588t_lcd_ctrl_write(unsigned char c)
{
	st7588t_lcd_i2c_transfer(c);
}
...
//
static void
st7588t_lcd_ctrl_address_set(unsigned int page, unsigned int col)
{
	st7588t_lcd_i2c_transfer(CMD_SET_PAGE | page);
	st7588t_lcd_i2c_transfer(CMD_SET_COLUMN_LOWER | (col & 0xf));
	st7588t_lcd_i2c_transfer(CMD_SET_COLUMN_UPPER | ((col >> 4) & 0xf));
}

static unsigned int
st7588t_lcd_ctrl_write_word(char word)
{
	int i;
	
	for (i = 0; i < FONT_COL_SIZE; i++)
		st7588t_lcd_i2c_transfer(Font8x5[(int)word][i]);
		
	return FONT_COL_SIZE;
}

static void
st7588t_lcd_ctrl_clear_screen(void)
{
	int i, j;
	
	for (i=0; i<ST7588T_PAGE_SIZE; i++)
	{		
		st7588t_lcd_ctrl_start();
		st7588t_lcd_ctrl_command(CMD_COMMAND_START);
		//st7588t_lcd_ctrl_command(0x38);
		st7588t_lcd_ctrl_address_set(i, 0);
		st7588t_lcd_ctrl_stop();
		st7588t_lcd_ctrl_start();
		st7588t_lcd_ctrl_command(CMD_DATA_START);
		for (j=0; j<ST7588T_COL_SIZE; j++)
			st7588t_lcd_ctrl_write(0x0);
		st7588t_lcd_ctrl_stop();
	}
}
...
//
static void
st7588t_lcd_ctrl_init(void)
{
	st7588t_lcd_ctrl_start();
	st7588t_lcd_ctrl_command(CMD_COMMAND_START);
	st7588t_lcd_ctrl_command(CMD_FUNCTION_SET | 0x1b); // MX=1, MY=1, PD=0, HI=1, H0=1
	st7588t_lcd_ctrl_command(CMD_COMMAND_RESET); // SW reset
	st7588t_lcd_ctrl_command(CMD_FUNCTION_SET | 0x18); // MX=1, MY=1, PD=0, HI=0, H0=0
	st7588t_lcd_ctrl_command(CMD_VLCD_RANGE); // low
	st7588t_lcd_ctrl_command(CMD_DISPLAY_CONFIG | 0xc); // display
	st7588t_lcd_ctrl_command(CMD_FUNCTION_SET | 0x19); // MX=1, MY=1, PD=0, HI=0, H0=1
	st7588t_lcd_ctrl_command(CMD_DISPLAY_CONFIG); // display
	st7588t_lcd_ctrl_command(CMD_BIAS_SYSTEM | 0x03);
	st7588t_lcd_ctrl_command(CMD_SET_VOP | 0x6f);
	st7588t_lcd_ctrl_stop();

	st7588t_lcd_ctrl_clear_screen();
}

//========================================================================  
// st7588t lcd misc dev driver
//========================================================================

#define ST7588T_LCD_MINOR 11

static ssize_t
st7588t_lcd_misc_write( struct file * file, const char __user * buf, 
				size_t count, loff_t *ppos )
{
	struct st7588t_lcd_dev *dev = file->private_data;
	...
	st7588t_lcd_ctrl_address_set(dev->cur_page, dev->cur_col);
	...
	for ( i = *ppos; count > 0; ++i, ++p, --count ) 
	{
		...
		if (dev->cur_col >= ST7588T_COL_SIZE)
		{
			dev->cur_page++;
		...
		dev->cur_col += st7588t_lcd_ctrl_write_word(c);
	}
	*ppos = i;
	return p - buf;
}
...
static ssize_t
st7588t_lcd_misc_read( struct file * file, char __user * buf,  
				size_t count, loff_t *ppos )
{
	struct st7588t_lcd_dev *dev = file->private_data;
	char __user *p = buf;
	...
	{
		char c;
		page = (i / ST7588T_PAGE_FONT_SIZE) % ST7588T_PAGE_SIZE;
		word = i % ST7588T_PAGE_FONT_SIZE;		
...
enum {
	ST7588T_LCD_INIT,
	ST7588T_LCD_CLEAR,		
	ST7588T_LCD_WRITE_PAGE,
	ST7588T_LCD_WRITE_PAGE_COL,
	ST7588T_LCD_LSHIFT_COL_OF_PAGE,
};

struct lcd_args {
	char string[64];
	unsigned char byte[ST7588T_COL_SIZE];
	int page;
	int s_col;
	int e_col;
};

static void st7588t_lcd_show_logo(struct st7588t_lcd_dev *dev);

static int
st7588t_lcd_misc_ioctl( struct inode * inode, struct file * file,
				unsigned int cmd, unsigned long arg )
{
	...
	switch ( cmd )
	{
	case ST7588T_LCD_INIT:
		st7588t_lcd_ctrl_init();
		st7588t_lcd_sw_reset(lcd_dev);
		st7588t_lcd_show_logo(lcd_dev);
		break;

	case ST7588T_LCD_CLEAR:
		st7588t_lcd_ctrl_clear_screen();
		st7588t_lcd_sw_reset(dev);
		break;

	case ST7588T_LCD_WRITE_PAGE:
		copy_from_user(&args, (struct lcd_args *)arg, sizeof(struct lcd_args));
	
		st7588t_lcd_ctrl_start();
		st7588t_lcd_ctrl_command(CMD_COMMAND_START);
		st7588t_lcd_ctrl_command(CMD_FUNCTION_SET); // MX=0, MY=0, PD=0, HI=0, H0=0
		st7588t_lcd_ctrl_address_set(args.page, 0);
		st7588t_lcd_ctrl_stop();
		st7588t_lcd_ctrl_start();
		st7588t_lcd_ctrl_command(CMD_DATA_START);
		...
		{
			st7588t_lcd_ctrl_write_word((args.string)[i]);
	...
	case ST7588T_LCD_WRITE_PAGE_COL:
		copy_from_user(&args, (struct lcd_args *)arg, sizeof(struct lcd_args));
		...
		for (i=0; i<ST7588T_COL_SIZE; i++)
		{
			st7588t_lcd_ctrl_write((args.byte)[i]);
			dev->col_data[args.page][i] = args.byte[i];
		}
		st7588t_lcd_ctrl_stop();
		break;

	case ST7588T_LCD_LSHIFT_COL_OF_PAGE:
		copy_from_user(&args, (struct lcd_args *)arg, sizeof(struct lcd_args));
		
		st7588t_lcd_ctrl_start();
		st7588t_lcd_ctrl_command(CMD_COMMAND_START);
		st7588t_lcd_ctrl_command(CMD_FUNCTION_SET); // MX=0, MY=0, PD=0, H1=0, H0=0
		st7588t_lcd_ctrl_address_set(args.page, args.s_col); 
		st7588t_lcd_ctrl_stop();
		st7588t_lcd_ctrl_start();
		st7588t_lcd_ctrl_command(CMD_DATA_START);
		...
		for (i = args.s_col; i <= args.e_col; i++)
		{
			...
			if (i == args.e_col)
			{
				...
				st7588t_lcd_ctrl_write(s_col_data);
			}
			else	
			{
				dev->col_data[args.page][i] = dev->col_data[args.page][i+1];
				st7588t_lcd_ctrl_write(dev->col_data[args.page][i+1]);
			}
		}
		st7588t_lcd_ctrl_stop();
		break;
...
static int
st7588t_lcd_misc_open( struct inode * inode, struct file * file )
{
	if (lcd_dev) 
...
const struct file_operations st7588t_lcd_misc_fops = {
	.write	= st7588t_lcd_misc_write,
	.read   = st7588t_lcd_misc_read,
	.ioctl	= st7588t_lcd_misc_ioctl,
	.open	= st7588t_lcd_misc_open,
};


//========================================================================  
// st7588t lcd dev driver
//========================================================================
#if 0
static int __devinit st7588t_lcd_probe(struct platform_device *pdev)
{
	lcd_dev = (struct st7588t_lcd_dev *) kzalloc(sizeof(struct st7588t_lcd_dev), GFP_KERNEL);
	if (!lcd_dev) {
		kfree(lcd_dev);
		return -ENOMEM;
	}
	...
	gpio_direction_output(PIN_SDA_GPIO, 1);
...
static int __devexit st7588t_lcd_remove(struct platform_device *pdev)   
{
	kfree(lcd_dev);
	return 0;
}

static struct platform_driver st7588t_lcd_driver = {
	.probe          = st7588t_lcd_probe,
	.remove         = __devexit_p(st7588t_lcd_remove),
	.suspend        = NULL,
	.resume         = NULL,
	.driver         = {
		.name   = "st7588t_lcd",
...
static void st7588t_lcd_show_logo(struct st7588t_lcd_dev *dev)
{
	...
	st7588t_lcd_ctrl_command(CMD_COMMAND_START);
	st7588t_lcd_ctrl_command(CMD_FUNCTION_SET); // MX=0, MY=0, PD=0, HI=0, H0=0
	st7588t_lcd_ctrl_address_set(0, 0);
	st7588t_lcd_ctrl_stop();
	st7588t_lcd_ctrl_start();
	st7588t_lcd_ctrl_command(CMD_DATA_START);
	...
	for (i=0; i<strlen(logo); i++, word++)
	{
		...
		st7588t_lcd_ctrl_write_word(logo[i]);
		dev->data[page][i%ST7588T_PAGE_FONT_SIZE] = logo[i];
...
/*
static void st7588t_lcd_scrolling_text(
	struct st7588t_lcd_dev *dev,
	unsigned int page, unsigned int s_col, unsigned int e_col)
{
	...
	st7588t_lcd_ctrl_address_set(page, s_col); 
	...
	for (i = s_col; i <= e_col; i++)
	{
		...
		else	
		{
			...
			st7588t_lcd_ctrl_write(dev->col_data[page][i+1]);
		}
	#ifdef DEBUG
...
static void st7588t_lcd_scan(struct work_struct *work)
{
	struct st7588t_lcd_dev *dev = container_of(work, struct st7588t_lcd_dev, work.work);
	const int delay = 10;
	
	st7588t_lcd_scrolling_text(dev, 4, 12, 119);
	
	schedule_delayed_work(&dev->work,delay);	
}
*/
static int __init
st7588t_lcd_init(void)
{
	...
	(lcd_dev->misc_dev).minor  = ST7588T_LCD_MINOR;
	(lcd_dev->misc_dev).name   = "cta_lcd";
	(lcd_dev->misc_dev).fops   = &st7588t_lcd_misc_fops;
	...
	gpio_request(PIN_SDA_GPIO, "cta_lcd");
	...
	//INIT_DELAYED_WORK(&lcd_dev->work, st7588t_lcd_scan);
	//schedule_delayed_work(&lcd_dev->work, 0);
	
	return 0;

	//return platform_driver_register(&st7588t_lcd_driver);
}

static void __exit
st7588t_lcd_exit(void)
{
	misc_deregister(&(lcd_dev->misc_dev));
	kfree(lcd_dev);
	//platform_driver_unregister(&st7588t_lcd_driver);
}

module_init(st7588t_lcd_init);
module_exit(st7588t_lcd_exit);
