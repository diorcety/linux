...
/* fix a race condition (first hit with OpenWRT) */
#define ETHDRV_BUGFIX
...
| Function Prototypes                                                          |
+=============================================================================*/
u8 (*monitor_wifi_status_hook)(void) = NULL;
void (*set_wifi_recover_hook)(u8 val) = NULL;
EXPORT_SYMBOL_GPL(monitor_wifi_status_hook);
EXPORT_SYMBOL_GPL(set_wifi_recover_hook);
...
#if defined(USE_TASKLET) && defined(USE_WORKQUEUE)
#error "Ony one of tasklet or workqueue can be select"
#endif
//#define USED_VLAN
...
static int cta_eth_rxint(rxctrl_t *prxc, int budget) IN_SEC_IRAM;
#ifndef USE_NAPI
static void cta_eth_deliver(struct net_device *sc) IN_SEC_IRAM;
#endif
#ifdef USE_NAPI
#define CTA_ETH_NAPI_WEIGHT 8
static int cta_eth_poll(struct napi_struct *napi, int budget) IN_SEC_IRAM;
#endif

static irqreturn_t cta_eth_isr(int irq, void *data) IN_SEC_IRAM;
...
| Functions                                                                    |
+=============================================================================*/
#ifdef CONFIG_P0_NO_PHY_REG
static const unsigned short fake_phy_regs[8]={
	BMCR_SPEED100|BMCR_FULLDPLX, 
	BMSR_100FULL|BMSR_LSTATUS,
	0x1234, 
	0x5678, 
	ADVERTISE_100FULL, /* 100FULL */ 
	LPA_100FULL, /* 100FULL */ 
	0, 0
};
/*!-----------------------------------------------------------------------------
 ...
 *      \return
 +----------------------------------------------------------------------------*/
static inline int cta_eth_mdio_fake(short phy,short reg,unsigned short *pval,int wr)
{
	//if ( 0 == phy )
	{
		if (!wr && pval)	//read only
			* pval = ( 8 > reg) ? fake_phy_regs[reg] : 0 ;
		return 1; //Capture
	}
	//return 0; //PASS
}
...
void cta_eth_mdio(short phy,short reg,unsigned short *pval,int wr)
{
	...
	{
#ifdef CONFIG_P0_NO_PHY_REG
		if (cta_eth_mdio_fake(phy, reg, pval, wr))
			return ;
#else
		internal = 0;
#endif
...
#endif
#ifdef USE_WORKQUEUE
	pcta_eth->wq = create_rt_workqueue("cheetah_eth");
	INIT_WORK( &pcta_eth->work,  (work_func_t) cta_eth_deliver );
#endif
...
#endif
#ifdef USE_WORKQUEUE
		queue_work(pcta_eth->wq, &pcta_eth->work);
#endif
#ifdef  USE_NAPI
...
#if !defined(ETHDRV_BUGFIX)
	if(ptxc->num_freetxd == 0)
	{
		cta_eth_txint(ptxc, 1);
		if(ptxc->num_freetxd == 0)
...
#if 0 // Cheetah ETH DMA does not have 16Mbytes limitation
...
#if defined (CONFIG_CHEETAH_SKB_RECYCLE_2K)
...
static void cta_eth_sw_monitor(unsigned long id)
{
	...
	if(rc == 2)
	{
		cta_eth_phy_reset(1,PHYID_MONT_EPHY);
	}
	else if(rc)
	{
	...
	mod_timer(&pcta_eth->timer, jiffies + 20);
}
...
static int mon_eth_drv_init(int unit)
{
	...
	pcta_eth->timer.expires = jiffies + 20;
...
#endif
#ifdef USE_WORKQUEUE
	flush_workqueue(pcta_eth->wq);
	destroy_workqueue(pcta_eth->wq);
#endif
	for(i=0;i< ETH_DEV_INIT; i++)
	{
		if(if_cta_eth_g[i]==0)
